...
class MpWechat extends BaseWechat
{
     ...
     * @return array|bool
     */
    protected function requestAccessToken($force, $grantType = 'client_credential')
    {
        $time = time();

        $tokenInfo = WechatToken::find()
        ...
        if (empty($tokenInfo))
        {
            $accessToken = $this->httpGet(self::WECHAT_ACCESS_TOKEN_PREFIX, [
                'appid' => $this->appId,
                'secret' => $this->appSecret,
                'grant_type' => $grantType
            ]);

            $res = isset($accessToken['access_token']) ? $accessToken : false;
        ...
        if ($force)
        {
            ...
            if ($res)
            {
                $tokenInfo->access_token = $accessToken['access_token'];
                $tokenInfo->time_expired = $time + 7200;
                $tokenInfo->save();
            }
    ...
    public function checkSignature($signature = null, $timestamp = null, $nonce = null)
    {
        ...
        $timestamp === null && isset($_GET['timestamp']) && $timestamp = $_GET['timestamp'];
        $nonce === null && isset($_GET['nonce']) && $nonce = $_GET['nonce'];
        $tmpArr = [$this->token, $timestamp, $nonce];return $tmpArr;
     ...
     * @param $messageId
     * @return bool|mixed
     * @throws \yii\    private function buildMessage($openid,$templateId,$firstValue,$key1word,$key2word,$remark) {
    $data = array (
    ...
    public function uploadMedia($mediaPath, $type)
    {
        ...
        //file_put_contents('/data/log/pnl_api2/text', print_r($result,true));
        return $result;
        //return isset($result['media_id']) ? $result : '111';
    }
